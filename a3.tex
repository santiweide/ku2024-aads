...
\textbf{Answer}
To verify $TAUTOLOGY \in co-NP$ we need to prove $\overline{TAUTOLOGY} \in NP$. We find that the $\phi$ could be transformed into a graph with input variables as the edges, and the negations ($\neg$), ANDs ($\vee$), ORs ($\wedge$) as vertices. Then the evaluation of the $\phi$ could be done in polynomial time as it is the same as finding paths on an acyclist graph from starting point to ending point.
